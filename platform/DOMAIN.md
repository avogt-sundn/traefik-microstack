...
The **Angular shell (host application)** and the **shared UI library** used by all microfrontends. Platform is the single entry point for the entire SPA — it loads and orchestrates the `partner` remotes at runtime via Native Federation, and provides the `platform/shared` library consumed by every MFE at build time.
...
The shell intentionally runs at the **lowest Traefik priority (100)** so that MFE containers (`/partner` at priority 120) take precedence.
...
```json
{
  ...
  "partner-edit":   "/partner-edit/remoteEntry.json"
}
```

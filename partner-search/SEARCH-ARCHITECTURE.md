...
### Loan Search Endpoint

`GET /api/loans/search` with 9 individual query params: `contractNumber`, `applicationNumber`, `partnerNumber`, `firstName`, `lastName`, `dateOfBirth`, `postalCode`, `city`, `vehicleLicensePlate`. No tokenization — each param maps directly to a SQL predicate. Returns `ContractSearchResponse`.
...
## Loan Search (Contrast)

The `SearchLoan` component (`loans/frontend/src/app/components/pages/search-loan/`) uses a traditional multi-field approach:

- A `FormGroup` with 9 validated fields across 3 rows: contract number, application number, partner number, first/last name, date of birth, postal code, city, vehicle license plate.
- Each field uses shared `ApiParameterValidators`. A cross-field validator requires at least one field with ≥ 3 characters.
- No tokenization, no autocomplete, no chip display.
- Calls `GET /api/loans/search` with the 9 params passed directly as query parameters.
- Results shown in a flat `mat-table`.

This pattern is appropriate when the search fields are heterogeneous (dates, numbers, free text) and the user is expected to know which field to fill.
